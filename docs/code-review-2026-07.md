...
- **`src/app/shared/modal/modal.tsx:159-211`** — no focus trap and no focus restoration on close (`aria-modal="true"` is present but not backed by real trapping), and there's no modal stack awareness, so a single Escape keypress closes every open modal simultaneously rather than the topmost one. **Deferred.** Suggested fix: add a focus-trap on mount / restore the previously-focused element on unmount, and gate the Escape handler on a shared modal-stack context so only the top modal responds.
